...
public class PlayerManager {
    ...
    private OrientationEventListener orientationEventListener;  //屏幕方向发生改变
    private int defaultTimeout=3000;  //播放等待超时
    private int screenWidthPixels;



    private boolean isShowing;
    private boolean portrait;//竖向
    private float brightness=-1;
    private int volume=-1;
    private long newPosition = -1;
    private long defaultRetryTime=5000;   //超时重连

    private PlayerStateListener playerStateListener;   //播放状态
    ...
    private boolean instantSeeking;  //seek拖动
    private boolean isDragging;   //拖动
...
//                        Toaster.show("download rate:" + extra);
                        if (playerStateListener != null){
                            playerStateListener.onNetWorkRate(extra);
                        }
                        break;
...
//            $.id(R.id.app_video_loading).visible();
    ...
    public interface PlayerStateListener{
        ...
        void onLoading();
        void onPlay();
        void onNetWorkRate(int netRate);//网速
    }
}
